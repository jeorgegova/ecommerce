          ...
          <tbody className="divide-y divide-gray-200 bg-white">
            {filtered.map((product) => (
              <tr key={product.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => router.push(`/admin/products/${product.id}`)}>
                ...
                <td className="whitespace-nowrap px-6 py-4 text-right text-sm text-gray-900 font-medium">₡{Number(product.base_price).toLocaleString("es-CR")}</td>
                  ...
                  <div className="flex items-center justify-center gap-2">
                    {product.sale_price ? (
                      <>
                        ...
                        <span className="text-xs font-medium text-green-600">₡{Number(product.sale_price).toLocaleString("es-CR")}</span>
                      </>
                    ) : (
          ...
          <div key={product.id} onClick={() => router.push(`/admin/products/${product.id}`)}
            ...
            <div className="p-3">
              ...
              <div className="mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-1">
                <span className="text-xs font-semibold text-gray-900">₡{Number(product.base_price).toLocaleString("es-CR")}</span>
                {product.sale_price ? (
