...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const cameFromProducts = referer.includes("/products") || referer.endsWith("/")
        ...
        <div className="mt-6 grid gap-10 lg:grid-cols-2">
          ...
          <div>
            <div className="flex items-start justify-between gap-4">
              <div>
                {categoryName && (
                  <p className="mb-1 text-xs font-medium uppercase tracking-wider text-gray-400">{categoryName}</p>
                )}
                <h1 className="text-2xl font-bold leading-tight text-gray-900 lg:text-[28px]">{product.name}</h1>
              </div>
              <FavoriteButton productId={product.id} />
            </div>

            {product.short_description && (
              <p className="mt-3 text-sm leading-relaxed text-gray-500">{product.short_description}</p>
            )}
            ...
            <div className="mt-6 rounded-2xl border border-gray-100 bg-gray-50/70 p-5">
              ...
              <div className="mt-4 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-500">
                ...
                {product.stock > 0 ? (
                  <>
                    <span className="text-gray-300">·</span>
                    <span className="font-medium text-green-600">En stock ({product.stock})</span>
                  </>
                ) : (
                  <>
                    <span className="text-gray-300">·</span>
            ...
            <div className="mt-6">
              <AddToCartButton productId={product.id} stock={product.stock} hasVariants={product.has_variants} unitPrice={Number(currentPrice)} />
            </div>

            <dl className="mt-8 grid grid-cols-2 gap-3">
              <div className="rounded-xl bg-gray-50 px-4 py-3">
                <dt className="text-[11px] font-medium uppercase tracking-wider text-gray-400">SKU</dt>
                <dd className="mt-0.5 text-sm font-medium text-gray-900">{product.sku}</dd>
              </div>
              {product.weight && (
                <div className="rounded-xl bg-gray-50 px-4 py-3">
              ...
              {(product.width || product.height || product.length) && (
                <div className="rounded-xl bg-gray-50 px-4 py-3">
                  <dt className="text-[11px] font-medium uppercase tracking-wider text-gray-400">Dimensiones</dt>
                  <dd className="mt-0.5 text-sm font-medium text-gray-900">
                    {[product.width, product.height, product.length].filter(Boolean).join(" × ")} cm
                  </dd>
                </div>
              )}
            </dl>
