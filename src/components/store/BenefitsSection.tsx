...
function BeneficioIcon({ item, align }: { item: Beneficio; align: "left" | "right" }) {
  return (
    <Image
      ...
      alt=""
      aria-hidden="true"
      className={`h-11 w-11 flex-shrink-0 object-contain transition-transform duration-200 ease-out group-hover:scale-110 lg:h-14 lg:w-14 ${
...
function BeneficioItem({ item, align }: { item: Beneficio; align: "left" | "right" }) {
  return align === "left" ? (
    <div className="group flex items-center justify-end gap-5 text-right lg:gap-6">
      <BeneficioTexto item={item} />
      <BeneficioIcon item={item} align="left" />
    </div>
  ) : (
    <div className="group flex items-center gap-5 text-left lg:gap-6">
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex justify-center lg:hidden">
          <Image
            ...
            className="w-48 drop-shadow-xl transition-transform duration-500 ease-out hover:scale-105 sm:w-56"
            priority
          />
        </div>
        ...
        <div className="mt-12 space-y-10 lg:hidden">
          {mobileItems.map((item) => (
            <div key={item.titulo} className="group flex items-center gap-5 text-left sm:gap-6">
              <BeneficioIcon item={item} align="right" />
              <BeneficioTexto item={item} />
            </div>
          ))}
        ...
        <div className="mt-12 hidden items-center gap-8 lg:mt-16 lg:grid lg:grid-cols-3">
          <div className="order-1 space-y-16">
            {leftItems.map((item) => (
              <BeneficioItem key={item.titulo} item={item} align="left" />
            ))}
          </div>
          ...
          <div className="order-2 flex justify-center">
            <Image
              src={Bolsabene}
              alt="Bolsa de café Amelatte"
              className="w-80 drop-shadow-xl transition-transform duration-500 ease-out hover:scale-105"
              priority
            />
          </div>
          ...
          <div className="order-3 space-y-16">
            {rightItems.map((item) => (
              <BeneficioItem key={item.titulo} item={item} align="right" />
            ))}
          </div>
        </div>
      </div>
