...
function MenuItemCard({ item, onClick }: { item: MenuItem; onClick?: () => void }) {
  return (
    <Link
      href={item.href}
      onClick={onClick}
      className="flex flex-col items-center gap-1 rounded-xl px-2 py-3 text-center transition-colors hover:bg-gray-50"
    >
      ...
      <p className="text-[11px] text-gray-400 leading-tight line-clamp-2">{item.description}</p>
    </Link>
  )
}
...
const buyerItems: MenuItem[] = [
  ...
  },
  {
    ...
    label: "Historial",
    description: "Productos que visitaste",
    icon: (
      <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  return (
    <div ref={menuRef} className="relative">
      <button
        ...
        className={`flex items-center gap-2 rounded-full border px-3 py-1.5 text-sm font-medium transition-all duration-200 motion-reduce:transition-none ${
          open
      ...
      >
        ...
        <svg
          className={`h-4 w-4 text-gray-400 transition-transform duration-200 motion-reduce:transition-none ${
            open ? "rotate-180" : ""
          }`}
          fill="none"
          viewBox="0 0 24 24"
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-2 w-[420px] origin-top-right animate-scale-in rounded-2xl border border-gray-200 bg-white p-3 shadow-xl shadow-gray-900/5 motion-reduce:animate-none">
          <div className="border-b border-gray-100 px-2 pb-3 pt-1">
          ...
          <div className="py-2">
            ...
            <div className="grid grid-cols-3 gap-1">
              {buyerItems.map((item) => (
                <MenuItemCard key={item.href} item={item} onClick={handleItemClick} />
              ))}
            </div>
          </div>
          ...
          <div className="border-t border-gray-100 pt-2">
            <button
              ...
              className="flex w-full items-center gap-3 rounded-xl px-2 py-2.5 text-sm font-medium text-red-600 transition-colors hover:bg-red-50"
            >
